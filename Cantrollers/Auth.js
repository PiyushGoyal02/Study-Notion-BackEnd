...
exports.signUp = async (req, res) => {
    try{
        ...
        const {firstName, lastName, email, password, acountType, confirmPassword, contactNumber, otp} = req.body;
        ...
        const databody = await User.create({firstName, 
            lastName, 
            email, 
            contactNumber, 
...
exports.login = async (req, res) => {
    try{
        ...
        if(await bcrypt.compare(password, CheckEmail.password)){
            let token = JWT.sign(JWTPayload, process.env.JWT_SERCET, {
                expiresIn:"2h",
            });
